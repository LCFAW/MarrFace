...
class Renderer():
    def __init__(self, cfgs):
        ...
        self.local_rank = dist.get_rank() if self.distributed else 0
        self.image_size = cfgs.get('map_size', 64)
        ...
        # dist_coeffs = torch.FloatTensor(dist_coeffs).to(self.device)
        t = torch.zeros(1,3, dtype=torch.float32).to('cuda')
    ...
    # project grid_3d to 2d(divide z), use in_param to transform it to 2d and normalize to (-1,1)
    def grid_3d_to_2d(self, grid_3d):
        b, h, w, _ = grid_3d.shape
        grid_2d = grid_3d / (grid_3d[...,2:] + 1e-8)
    ...
    # NOTE  transform point from camera coordinate to world coordinate, first rotate, next translate
    ...
    # NOTE  transform point from  world  coordinate to camera coordinate, first translate, next rotate
    ...
    def render_yaw_spec(self, im, depth, view):
        b, c, h, w = im.shape
        grid_3d = self.depth_to_3d_grid(depth)
        grid_3d = grid_3d.reshape(b,-1,3)
...
class Renderer_black():
    ...
    def depth_to_3d_grid(self, depth):
        ...
        grid_2d = get_grid(b, h, w, normalize=False).to(depth.device)  # Nxhxwx2
        depth = depth.unsqueeze(-1)
        # concat a one matrix in the final dimension
        grid_3d = torch.cat((grid_2d, torch.ones_like(depth)), dim=3)
        # mutiply camera inna-param and depth to get the 3d grid
        grid_3d = grid_3d.matmul(self.inv_K.to(depth.device).transpose(2,1)) * depth
        return grid_3d
    ...
    def render_yaw_spec(self, im, depth, view, rx=0,rz=0):
        b, c, h, w = im.shape
        grid_3d = self.depth_to_3d_grid(depth)
        grid_3d = grid_3d.reshape(b,-1,3)
        ...
        ri = torch.FloatTensor([rx, ri, rz]).to(im.device).view(1,3)
        rot_mat_i, _ = get_transform_matrices(ri)
        grid_3d_i = self.rotate_pts(grid_3d, rot_mat_i.repeat(b,1,1))
