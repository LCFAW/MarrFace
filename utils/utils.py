...
def save_images(out_fold, imgs, name =None, prefix='', suffix='', sep_folder=True, ext='.png'):
    if sep_folder:
        out_fold = os.path.join(out_fold, suffix)
    xmkdir(out_fold)
    prefix = prefix + '_' if prefix else ''
    ...
    for i, img in enumerate(imgs):
        ...
        if name == None:
            ...
            cv2.imwrite(os.path.join(out_fold, name[i]+ext), im_out)


def save_images_switch(out_fold, imgs, name =None, prefix='', suffix='', sep_folder=True, ext='.png'):
    if sep_folder:
        out_fold = os.path.join(out_fold, suffix)
    xmkdir(out_fold)
    prefix = prefix + '_' if prefix else ''
    ...
    for i, img in enumerate(imgs):
        ...
        if name == None:
            ...
            cv2.imwrite(os.path.join(out_fold, name[i]+'_'+ name[(i+1)%(batch-1)]+ext), im_out)


def save_npy(out_fold, matrixs, name =None, prefix='', suffix='', sep_folder=True, ext='.npy'):
    if sep_folder:
        out_fold = os.path.join(out_fold, suffix)
    xmkdir(out_fold)
    prefix = prefix + '_' if prefix else ''
    ...
    for i, matrix in enumerate(matrixs):
        ...
        np.save(os.path.join(out_fold, name[i]+ext), matrix)            
...
def get_coordinate_tensors(x_max, y_max, part_map):
    x_map = np.tile(np.arange(x_max), (y_max,1))/x_max*2 - 1.0
    y_map = np.tile(np.arange(y_max), (x_max,1)).T/y_max*2 - 1.0

    x_map_tensor = torch.from_numpy(x_map.astype(np.float32)).to(part_map.device)
    y_map_tensor = torch.from_numpy(y_map.astype(np.float32)).to(part_map.device)
...
def get_center(part_map):
    ...
    h,w = part_map.shape
    x_map, y_map = get_coordinate_tensors(h,w, part_map)
